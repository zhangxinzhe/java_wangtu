...
public class WarningMailUtil {
    ...
    private static WarningMailUtil mailUtil = new WarningMailUtil();

    private static final Logger log = LoggerFactory.getLogger(WarningMailUtil.class);

    private JavaMailSenderImpl mailSenderImpl;

    // 邮件host
    private static final String MAIL_HOST = "mail_warn.host";
    // 邮件的用户名
    private static final String MAIL_USERNAME = "mail_warn.username";
    // 邮件的密码
    private static final String MAIL_PASSWORD = "mail_warn.password";
    // 邮件的发送方
    private static final String MAIL_FROM = "mail_warn.from";
    // 要过滤的邮箱
    private static final String MAIL_FILTER = "mail.filter";

    public static WarningMailUtil instance() {
        return mailUtil;
    }
    ...
    public void send(String email[], String subject, String content, String person) {
        sendExecutService.submit(new SendExecutor(email, subject, content, person));
    }
    ...
    public void send(String subject, String content) {
        String person = NetstudyConfig.getParam(BaseConstants.SITE_HOME_SITE_NAME) + "_"
                + NetstudyConfig.getParam(BaseConstants.DOMAIN_HOME);
        String[] emails = NetstudyConfig.getParam("mail.receive").split(",");
        if (emails != null && emails.length <= 0) {
            return;
        }
        send(emails, subject, content, person);
    }
