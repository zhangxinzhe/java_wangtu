...
public class Uploader {
    ...
    // 保存路径
    private String savePath = "upload/editor";
    // 文件允许格式
    ...
    public void upload() throws Exception {
        ...
        String savePath = OSUtil.getTempDir() + path;
        ...
        try {
            ...
            while (fii.hasNext()) {
                ...
                if (!fis.isFormField()) {
                    ...
                    String fileUrl = savePath + "/" + this.fileName;
                    path += "/" + this.fileName;
                    this.url = getUrlFolder(fileUrl);
    ...
    public void uploadBase64(String fieldName) {
        ...
        this.fileName = this.getName(fieldName);
        String fileUrl = savePath + "/" + this.fileName;
        this.url = getUrlFolder(fileUrl);
        ...
        try {
            ...
            ro.close();
            this.state = this.errorInfo.get("SUCCESS");
            this.url = getUrlFolder(this.url);
            FileSystemUtil.saveFile(outFile, path + "/" + this.fileName);
            outFile.delete();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    private String getFolder(String path) {
        ...
        File dir = new File(OSUtil.getTempDir() + path);
        if (!dir.exists()) {
            try {
                dir.mkdirs();
            }
     ...
     * @return
     */
    private String getUrlFolder(String path) {
        return path.replace(OSUtil.getTempDir(), Config.getParam(BaseConstants.DOMAIN_UPLOAD_FILE));
